...
app.route('/', (state, emit) => {
  ...
  return html`
    <main>
      ${state.participants.map(archive =>
        state.cache(NetworkInfo, archive.url).render(archive)
      )}
      ${
        !state.readOnly && Object.keys(state.videos).length === 0
  ...
  function renderVideos() {
    return html`<div style="display: flex; flex-direction: row">${state.firsts.map(
      url => renderVideo(state.videos[url])
    )}</div>`
  }
