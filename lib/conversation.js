...
class Conversation extends EventEmitter {
  ...
  async _setState() {
    ...
    if (this.pendingContributor != null) {
      if (contributorInfos.find(p => p.url === this.pendingContributor)) {
    ...
    await Promise.all(contributors.map(p => p.ready))
    ...
    const contributorVideos = contributors.map(p => p.videos)
